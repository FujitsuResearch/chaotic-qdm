...
FOLDER_NAME=demo_train_circular_chaotic
...
SCRAMB='Ising_nearest'
EVOL='full,trotter'

for delta in 0.02
do
for STEPS in 50 # 20 steps is sufficient for convergence in this simple case, but we can also try more steps
do
...
taskset -c 0-19 python $RUN --bin $BIN --batch_size $BATCH_SIZE --vendi_lambda $VENDI_LAMBDA --gen_circuit_type $ANSATZ  --threads $THREADS --rseed $SEEDS --n_outer_epochs $EPOCHS --dat_name $DATNAME --bloch $BLOCH --n_diff_steps $STEPS --n_qubits $NQUBITS --n_ancilla $NANCILA --lr $LR --mag $MAG --save_dir $SAVE --input_type $INPUT --n_train $NTRAIN --n_test $NTEST --n_layers $GEN_LAYERS --dist_type $DIST_TYPE --scramb $SCRAMB --type_evol $EVOL --delta_t $delta --n_pj_qubits $N_PJ
done
done
done
done